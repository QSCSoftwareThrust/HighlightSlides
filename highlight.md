...
You are running in the `HighlightSlides` host workflow. Do not use Docker,
Podman, or the container workflow. Use the already activated Python environment
and the `highlight` command available on `PATH`.

The user will ask for a highlight slide input from a paper, usually with:
...
reuses the complete project tree under `work/` automatically.
